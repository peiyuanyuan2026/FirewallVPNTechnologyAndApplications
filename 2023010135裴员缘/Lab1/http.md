...
1. HTTP 请求报文由哪几部分构成？请按顺序列出：
   ...
   > 答：方法，URI,协议，版本号，请求头，请求体

2. 状态码 `404` 代表什么含义？状态码 `500` 和 `503` 有什么区别？

   > 答：404：服务器无法找到请求的资源， URL 输入错误、资源已删除或路径不存在；500：服务器在处理请求时发生了未知错误，服务器端代码 / 配置异常；503：服务器暂时无法处理请求，通常是过载、维护或资源不足，属于临时状态

3. GET 与 POST 方法的主要区别是什么？各适用于什么场景？

   > 答：GET 数据拼在 URL，可见、长度有限、可缓存、幂等；POST 数据放请求体，隐蔽、无长度限制、不缓存、非幂等；GET用于获取、查询数据，POST用于提交、修改数据

4. HTTP 与 HTTPS 有什么区别？HTTPS 使用了什么机制来保护数据？

   > 答：HTTP 是明文传输，数据易被窃听、篡改，默认使用 80 端口，需要 CA 颁发的 SSL/TLS 证书，用于验证服务器身份，防止中间人攻击；HTTPS 是加密传输，数据更安全，默认使用 443 端口，无身份验证机制
   HTTPS使用对称/非对称加密，数字证书以及哈希函数保护数据

5. 既然 HTTPS 已经加密，为什么浏览器开发者工具仍然能看到请求和响应的明文内容？

   > 答：因为 HTTPS 加密的是网络传输过程，不是浏览器本地内容。浏览器是通信的一端，它先解密拿到明文，再展示给开发者工具，所以能看到明文。
